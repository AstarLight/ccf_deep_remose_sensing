...
def get_unet0():
    ...
    crop9 = Cropping2D(cropping=((32, 32), (32, 32)))(conv9)
...
def is_pos_label(im):
    sum = im.sum()
    if (float(sum) / float(img_rows*img_cols) > 0.2):
        return True
    else:
        return False
...
def batch_generator(X, y, batch_size, horizontal_flip=False, vertical_flip=False, swap_axis=False):
    while True:
        ...
        yield X_batch, y_batch[:, :, 32:32 + img_rows - 64, 32:32 + img_cols - 64]
...
if __name__ == '__main__':
    ...
    f = h5py.File(os.path.join(data_path, 'chen_train_water5.h5'), 'r')
